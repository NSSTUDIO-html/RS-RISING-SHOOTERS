...
const texture1 = textureLoader53.load('https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTjpjgxgG06_soGXCJnlXWyh2HgPHY9bCTWfyuTyXodLx6K1SEKXRcPaq8&s=10');
   ...
   const texture5 = textureLoader53.load('https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTwARDqWEUA1Oc6_KqnWcs5bKKyoCI6OKLHMRMiXE9SLUtx1jEmH5vMfspA&s=10');
    ...
    const armysuit = new THREE.Group();
   ...
   const barrelMaterial4 = new THREE.MeshPhongMaterial({ map:texture1 ,metalness:0.85,roughness:0.0,envMap:scene.environment,transparent:true,opacity:0.8});
...
armysuit.add( sphere );
  ...
  armysuit.add(c25);
   const c24u = new THREE.Mesh(bodyGeometry, barrelMaterial3);
    c24u.position.set(2,-0.5,2.2); // Position it below the barrel
    c24u.scale.set(0.2,0.5,0.5);
   c24u.rotation.x = Math.PI/2.2;
   c24u.rotation.z = Math.PI/3;
  armysuit.add(c24u);
   const c25u = new THREE.Mesh(bodyGeometry, barrelMaterial3);
    c25u.position.set(-2,-0.5,2.2); // Position it below the barrel
    c25u.scale.set(0.2,0.5,0.5);
   c25u.rotation.x = Math.PI/2.2;
   c25u.rotation.z = -Math.PI/3;
  armysuit.add(c25u);
  ...
  armysuit.add(body2s5s);
   //shouldhands
 export  const rhand = new THREE.Group();
...
rhand.position.set(0, -12,-10);
    //currentGun.rotation.x = -Math.PI;
//currentGun.position.set(-0.4, 22, 49.8);
  ...
  armysuit.add(bodyll341);
   ...
   //binoculars
   const bodyll31 = new THREE.Mesh(headGeometry, barrelMaterial);
    bodyll31.position.set(0,3,1.5); // Position it below the barrel
    bodyll31.scale.set(0.5,0.5,0.5);
   bodyll31.rotation.x = Math.PI/3;
  
  armysuit.add(bodyll31);
   const bodyll314 = new THREE.Mesh(headGeometry, barrelMaterial);
    bodyll314.position.set(0,4,3.5); // Position it below the barrel
    bodyll314.scale.set(0.5,0.5,0.5);
   bodyll314.rotation.x = Math.PI/3;
   bodyll314.rotation.z = Math.PI/2;
  
  armysuit.add(bodyll314);
   
   const bodypp = new THREE.Mesh(bodyGeometry, barrelMaterial);
    bodypp.position.set(1.5,4,3.5); // Position it below the barrel
    bodypp.scale.set(0.5,0.5,0.5);
   bodypp.rotation.x = Math.PI;
   bodypp.rotation.z = Math.PI;
  
  armysuit.add(bodypp);
   const bodyppq = new THREE.Mesh(bodyGeometry, barrelMaterial);
    bodyppq.position.set(-1.5,4,3.5); // Position it below the barrel
    bodyppq.scale.set(0.5,0.5,0.5);
   bodyppq.rotation.x = Math.PI;
   bodyppq.rotation.z = Math.PI;
  
  armysuit.add(bodyppq);
   const bodykk = new THREE.Mesh(bodyGeometry, barrelMaterial4);
    bodykk.position.set(1.5,4,3.7); // Position it below the barrel
    bodykk.scale.set(0.4,0.4,0.55);
   bodykk.rotation.x = Math.PI;
   bodykk.rotation.z = Math.PI;
  
  armysuit.add(bodykk);
   const bodyppq1 = new THREE.Mesh(bodyGeometry, barrelMaterial4);
    bodyppq1.position.set(-1.5,4,3.7); // Position it below the barrel
    bodyppq1.scale.set(0.4,0.4,0.55);
   bodyppq1.rotation.x = Math.PI;
   bodyppq1.rotation.z = Math.PI;
  
  armysuit.add(bodyppq1);
   ...
   const barrel = new THREE.Mesh(barrelGeometry, barrelMaterial3);
    ...
    barrel.position.set(-0.5,2.5,0);
  barrel.scale.set(0.5,0.4,0.5);
  w.add(barrel);
  // const bodyGeometry = new THREE.BoxGeometry(3, 3, 10);
   ...
   const b11 = new THREE.Mesh(bodyGeometry, bodyMaterial);
    b11.position.set(0,2.5,0); // Position it below the barrel
    b11.scale.set(1.2,0.5,0.3);
  w.add(b11);
   const bodyGeometry1 = new THREE.BoxGeometry(3, 3, 10);
    const bodyMaterial1 = new THREE.MeshStandardMaterial({ color:0xffffff,metalness:0.85,roughnessMap:0.0,envMap:scene.environment});
    
   const b1 = new THREE.Mesh(bodyGeometry1, bodyMaterial1);
    b1.position.set(0,2.7,0); // Position it below the barrel
    b1.scale.set(1.1,0.5,0.25);
  w.add(b1);
   const b22 = new THREE.Mesh(bodyGeometry1, barrelMaterial6);
    b22.position.set(1,2.5,0); // Position it below the barrel
    b22.scale.set(0.8,0.4,0.2);
   b22.rotation.x =Math.PI/2;
   b22.rotation.z = Math.PI/2;
  w.add(b22);
  ...
  const material11 = new THREE.MeshPhongMaterial({ color:0x000000});
   ...
   // Lighting
    ...
    //const ambientLight3 = new THREE.AmbientLight(0xffffff, 3);
 //   w.add(ambientLight3);
  ...
  armysuit.add(w);
   ...
   const ambientLight1 = new THREE.AmbientLight(0x000000, 2);
    armysuit.add(ambientLight1);
  armysuit.position.y = 25;
 export  const a2 = new THREE.Group();
  a2.add(armysuit);
   scene.add(a2);
